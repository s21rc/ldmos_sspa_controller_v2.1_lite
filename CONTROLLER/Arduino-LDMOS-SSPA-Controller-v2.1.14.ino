...
/*  Arduino Code (Version 2.1.14) for S21RC LDMOS Controller V1.x. PCB.                        */
...
/* no software protection except Temperature.                                                  */
 ...
 * 2.1.14 Lite (7 Jul 2024)
 * Bug fix for PEP hold
 * Gradual decay added for watt graph.
 * 
 * 
 * 2.1.13 Lite (14 March 2024):
 * Exponentially Weighted Moving Average used for the SWR and Power display
 * Efficiency counter removed as giving unstable reading
 *
 *
 * 2.1.12 Lite (17 Feb 2024):
 * SWR calculation fine tuned, working OK now.
 *
 *
...
*/
String controller_version = "Controller V 2.1.14";
...
bool dtemp2_status = 0; // 1 if second digital sensor present, 0 for one sensor.
...
float calibrationP;
int calibPower;
float Vdiode = 0.3; // if we assume FWD voltage diode = 0,3v
...
float power_fwd = 0.0; // power forward (watts)
float power_ref = 0.0; // power reflected (watts)
float currentREF = 0.0;
float currentFWD = 0.0;
float raw_fwd = 0.0; // power forward for SWR calculation
float raw_ref = 0.0; // power reflected for SWR calculation
...
float SWR = 0.0;         // SWR
float power_ratio = 0.0; // Power ratio P refl/P forward
...
{
    float currentV;
    int tmp_value = analogRead(VCC); // discarding first read
...
{
    currentFWD = 0.0;
    int tmp_value = analogRead(PWR_FWD); // discarding first read
    currentFWD = analogRead(PWR_FWD);
    currentFWD = currentFWD * ref_vdd;
    return currentFWD;
}
...
{
    currentREF = 0.0;
    int tmp_value = analogRead(PWR_REF); // discarding first read

    currentREF = analogRead(PWR_REF);
    currentREF = currentREF * ref_vdd;
    return currentREF;
}
...
void read_power()
{
    ...
    if (power_ref > 0.0 && power_fwd > 0.1)
    {
        power_ratio = power_ref / power_fwd; // Theory corrected
        SWR = abs((1 + sqrt(power_ratio)) / abs(1 - sqrt(power_ratio)));
        SWR = swrFilter.filter(SWR);
    }
    else
        SWR = 1.0;
...
void trigger9()
{
    ...
    myNex.writeStr("page1.t17.txt", controller_version);
    display_page = 1;
}
...
{ // 0A
    ...
    graph_maxWatt = myNex.readNumber("page1.s0.val");
    graph_maxSwr = myNex.readNumber("page1.s1.val");
    graph_maxTemp = myNex.readNumber("page1.s2.val");
    protection_po = myNex.readNumber("page1.s3.val");
    protection_swr = myNex.readNumber("page1.s4.val");
    protection_temp = myNex.readNumber("page1.s5.val");
    T_pepHOLD = myNex.readNumber("page1.s6.val");
    PWM_FREQ_KHZ = myNex.readNumber("page1.s7.val");
    calibPower = myNex.readNumber("page1.s8.val");
    calibvoltADC = myNex.readNumber("page1.s9.val");
    strcpy(new_callsign, (char *)myNex.readStr("page1.st10.txt").c_str());
    ...
    display_page = 1;
    delay(1000);
    myNex.writeStr("page1.st21.txt", "SETTINGS SAVED");
}
...
void trigger16()
{                     // 10
    calibvoltADC = 1; // reset to 1
}

void powerCalibration()
{

    calibrationP = (calibvoltADC * ref_vdd) + Vdiode;
    calibrationP = sq(calibrationP);
    ...
    while (1)
    {
        ...
        if (display_page == 2)
        {
            ...
            if ((millis() - lastDiagRequest) >= DiagdelayInMillis)
            {
                ...
                if (power_ref > 0.0 && power_fwd > 0.0)
                {
                    power_ratio = power_ref / power_fwd; // Theory corrected
                    // power_ratio = power_fwd / power_ref;
                    SWR = abs((1 + sqrt(power_ratio)) / abs(1 - sqrt(power_ratio)));
                }
                else
                {
                ...
                myNex.writeNum("D10.val", calibvoltADC);       // calib ADC
                myNex.writeNum("D11.val", calibPower);         // calib watt
                myNex.writeNum("D9.val", calibrationP * 1000); // CALIBRATION FACTOR
                myNex.writeNum("D0.val", raw_fwd / ref_vdd);   // FWD ADC
                myNex.writeNum("D2.val", raw_ref / ref_vdd);   // REF ADC
                myNex.writeNum("D1.val", power_fwd);
                myNex.writeNum("D3.val", power_ref);

                int swr_disp = (SWR * 10) + 0.5;
                myNex.writeNum("Dx2.val", swr_disp);
