/*                                                                                             */
/*  Arduino Code (Version 2.1.08) for S21RC LDMOS Controller V1.x. PCB.                        */
...
/*                                                                                             */
/* For PCB Version 1.x, Code V2.1.07 Lite version custom built based over CODE V 2.0 for DF1JM */

/* Simple graphical interface to show the status of the SSPA only,                            */
/* no software protection except Tempareture.                                                  */


/* PCB V1.6: https://github.com/s21rc/Arduino-LDMOS-SSPA-Controller                           */
/* Arduino and Nextion Code for V2.1.x Lite:                                                  */
...
raw_ref = ref_now();
 ...
 if(raw_fwd > 20){         // only correct for diode voltage when more than zero
          power_fwd = raw_fwd + Vdiode;
          power_fwd = power_fwd*power_fwd;
          power_fwd = power_fwd/calibrationP;
        } else power_fwd=0;
        
        if(raw_ref > 20){         // only correct for diode voltage when more than zero
  ...
  if(millis() > (lastTpep + T_pepHOLD)) power_fwd_max = power_fwd; // clear the peak after hold time
  
  if (Veff!=0 && Iprot!=0) Eff=100*(power_fwd/(Veff*Iprot));
...
void display_power(bool active){
  ...
  if(active){
    if((millis() - lastPRequest) >= PdelayInMillis){
        ...
        if (swr_display < 10) swr_display = 10 ; // SWR cannot be lower than 1.0
...
void fanspeed(){
  if(temperature<25.0) PWM=20;
  else if(temperature>25.0 && temperature<30.0) PWM=40;
  else if(temperature>30.0 && temperature<40.0) PWM=60;
  else if(temperature>=40.0 && temperature<45.0) PWM=70;
  else if(temperature>=45.0 && temperature<50.0) PWM=80;
  else if(temperature>=50.0) PWM=100;
...
void read_temp(){
  ...
  if(temperature>= float(protection_temp)){
    setPwmDuty(100);
    protection_en(true, true, 7);
    error_temp_status = true;
    }
  
 if(temperature<=(protection_temp-2) && error_temp_status){
...
void trigger15(){ //0F
  strcpy(new_callsign,(char*) myNex.readStr("st10.txt").c_str());
  if(strcmp(callsign, new_callsign)!=0) strcpy(callsign,new_callsign); 
...
void loop() {
    ...
    if(display_page==1){
      ...
      if(PTT_status){
        ...
        maxcalibV = (analogRead(PWR_FWD));
        if (maxcalibV>calibvoltADC) calibvoltADC=maxcalibV;
        myNex.writeNum("s9.val", calibvoltADC);
